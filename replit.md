...
- **ハイヤールーム**: あさひ・Hドラ・Hアキの3者チャット空間（/hayroom, /api/hayroom）。DB: tryroom_messages, Drizzle: hayroomMessages。
- **自律ループ**: Dドラ・Dアキ・アキ（アバター）の自律対話。DB: triroom_messages, Drizzle: loopMessages。制御API: /api/loop（GET/POST）。UI非表示、APIのみ。
