...
2. dev_recordsのactiveレコードを確認する（`SELECT * FROM dev_records WHERE status = 'active' ORDER BY priority DESC`）
3. agent_session_contextから前回の文脈を復元する
4. 「過去のバグから学んだルール」セクションを再確認する
