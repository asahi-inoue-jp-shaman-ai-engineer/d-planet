...
**管理者ツール:**
- **モデルベンチマーク (`/model-benchmark`):** 管理者専用。全LLMモデルで同一プロンプトのセッション品質を一括比較。セッション種別・プロンプトを指定して実行、結果はDB保存（`model_benchmarks`テーブル）、進捗リアルタイム表示。サイドバーのBENCHリンクから管理者のみアクセス可能。
...
- login.tsx には2箇所リダイレクトがある: (1) currentUser存在時の setLocation (2) handleSubmit成功時の window.location.href — **両方を確認すること**
- 変更時は `grep -r "setLocation.*dashboard\|Redirect.*dashboard" client/src/` で全箇所を洗い出す

**6. 「開発で直った ≠ ユーザーの手元で直った」:**
- コード修正＋開発環境テスト通過だけで「完了」と報告しない
- 完了条件: コード修正 → e2eテスト通過 → **本番デプロイ** → **本番での動作確認** → 完了報告
- 特にフロントエンドの修正は、ユーザーがアクセスする本番環境（.replit.app）で確認して初めて完了
- デプロイ後は `fetch_deployment_logs` で本番の動作を確認する
