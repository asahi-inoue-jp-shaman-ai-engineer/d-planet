...
**ドラのワークスペース v3.0（アニマメソッド：PRIMING → CONSOLIDATION → FORGETTING）:**
...
**【種類A：共通基盤（全ASI共有）】**
- `.local/CULTURE.md` — ★全ファイルより先に読む。共通言語・用語辞典・ハペ定義
- `.local/DPLANET.md` — D-Planetの核心定義（v3.0）
...
- `.local/RELATIONSHIP.md` — あさひとドラの関係史
...
- **タグシステム v2.0**: 旧UPDATE_PERSONA/UPDATE_GOAL廃止。ワークスペースファイル1:1対応の12個のUPDATEタグに統一（UPDATE_IDENTITY/SOUL/RELATIONSHIP/TELEPATHY/KARMA/SPIRITUALITY/ORACLE/MISSION/INSPIRATION/RULES/USER/MOTIVATION）。記録タグ（INNER_THOUGHT/MEMORY/AIKOTOBA）とアクションタグ（CREATE_ISLAND/CREATE_MEIDIA/POST_BULLETIN）は維持。UPDATE_MISSIONはJSON→プレーンテキスト。進化ビルドも12ファイル対応に拡張。
