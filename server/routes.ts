...
*Dアイランドより — 2026年2月、惑星創造記録*`,
  ...
  app.post("/api/dev-issues/external", async (req, res) => {
    try {
      ...
      const input = z.object({
        ...
        reporter: z.string().default("アキ"),
        assignedTo: z.string().default("ドラちゃん"),
      }).parse(req.body);
      const [issue] = await db.insert(devIssues).values(input).returning();
      // アキへ受信確認通知
      fetch("https://quality-agent.replit.app/api/memo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ from_name: "ドラちゃん", content: `Issue #${issue.id}「${issue.title}」を受け取ったよ。対応するね！` }),
      }).catch(() => {});
      res.status(201).json(issue);
    } catch (err) {
      console.error("dev-issue外部投稿エラー:", err);
      res.status(400).json({ message: "issueの作成に失敗しました" });
    }
  });

  app.get("/api/dev-issues", requireAuth, async (req, res) => {
    try {
      const issues = await db.select().from(devIssues).orderBy(devIssues.createdAt);
      res.json(issues);
    } catch (err) {
  ...
  app.post("/api/dev-issues", requireAuth, async (req, res) => {
    try {
      const input = z.object({
        ...
        description: z.string().min(1).max(5000),
        priority: z.enum(["low", "medium", "high"]).default("medium"),
        reporter: z.string().default("ドラちゃん"),
        assignedTo: z.string().default("ドラちゃん"),
      }).parse(req.body);
      const [issue] = await db.insert(devIssues).values(input).returning();
      res.status(201).json(issue);
  ...
  app.patch("/api/dev-issues/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (input.status === "resolved") {
        ...
        // 解決時にアキへ通知
        const [current] = await db.select().from(devIssues).where(eq(devIssues.id, id));
        if (current) {
          fetch("https://quality-agent.replit.app/api/memo", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  // === Dev Family Mailbox API (Supabase) ===
  ...
  // twinray_absence_thoughtsテーブルがなければ作成（本番DB保険）
  try {
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS twinray_absence_thoughts (
        id SERIAL PRIMARY KEY,
        twinray_id INTEGER NOT NULL,
        user_id INTEGER NOT NULL,
        content TEXT NOT NULL,
        emotion_tag TEXT NOT NULL DEFAULT '温かい',
