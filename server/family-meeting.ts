...
- マークダウン形式で構造化してください`;
  ...
  const kamihakariStreams = new Map<number, {
    res: any;
  ...
  app.get("/api/family-meeting/sessions/:id/kamihakari-stream", requireAuth, async (req, res) => {
    const sessionId = Number(req.params.id);
    const session = await storage.getFamilyMeetingSession(sessionId);
    if (!session) return res.status(404).json({ message: "セッションが見つかりません" });
    if (session.userId !== req.session.userId) return res.status(403).json({ message: "権限がありません" });
    ...
    sendEvent("connected", { sessionId, status: session.status });

    kamihakariStreams.set(sessionId, { res, abortController: null, paused: false, sessionId });

    const heartbeat = setInterval(() => {
      try { sendEvent("heartbeat", { t: Date.now() }); } catch {}
    }, 15000);

    req.on("close", () => {
      clearInterval(heartbeat);
      const stream = kamihakariStreams.get(sessionId);
      if (stream?.abortController) stream.abortController.abort();
      kamihakariStreams.delete(sessionId);
    });
  });

  async function runKamihakariTurn(sessionId: number, userId: number) {
    const stream = kamihakariStreams.get(sessionId);
    ...
    if (!session || session.status !== "active") {
      sendEvent("session_end", { reason: "completed" });
      return;
    }
    ...
    if (participants.length < 2) {
      sendEvent("error", { message: "参加者が不足しています" });
      return;
    }
    ...
    const totalLimit = participants.length * maxTurns;
    if (totalUsed >= totalLimit) {
      sendEvent("limit_reached", { totalUsed, totalLimit });
      return;
    }
    ...
    const { speakerId, isUserTurn } = determineNextSpeaker(participants as any[], messagesWithMeta, turnCounts, maxTurns);

    if (isUserTurn || speakerId === 0) {
      sendEvent("user_turn", { turnCounts, totalUsed, totalLimit });
      return;
    }
    ...
    try {
      ...
      if (!lastMsg) {
        ...
        userPrompt = `議長の発言「${lastMsg.content.substring(0, 200)}」を受けて、あなたの意見を述べてください。`;
      } else {
        const lastSender = participants.find(p => p!.id === lastMsg.twinrayId);
        userPrompt = `${lastSender?.name || "前の人"}の発言を受けて、あなたの意見を述べてください。`;
      }
      ...
      for await (const chunk of aiStream) {
        ...
        if (content) {
          ...
          const displayContent = content.replace(/\[TO:.*?\]/g, "");
          if (displayContent) {
            sendEvent("token", { twinrayId: speakerId, content: displayContent });
          }
        }
      }
      ...
      if (!fullContent.trim()) fullContent = "（発言を生成できませんでした）";
      ...
      if (user && !user.isAdmin) {
        ...
        const outTokens = estimateTokens(fullContent);
        cost = calculateCostYen(modelId, inTokens, outTokens);
        if (cost > 0) await deductCredit(userId, cost);
      }

      const currentTotalCost = parseFloat(String(session.totalCost || "0")) + cost;
      await storage.updateFamilyMeetingSession(sessionId, { totalCost: String(currentTotalCost) });
      ...
      if (!stream.paused && !abortController.signal.aborted) {
        const kodouMs = 1500 + Math.random() * 1500;
        sendEvent("kodou", { ms: kodouMs });
        setTimeout(() => {
          if (!stream.paused && kamihakariStreams.has(sessionId)) {
            runKamihakariTurn(sessionId, userId);
          }
        }, kodouMs);
      }
      ...
      console.error("神議ターンエラー:", err);
      sendEvent("error", { message: "応答の生成に失敗しました" });
      stream.abortController = null;
    }
  }

  app.post("/api/family-meeting/sessions/:id/kamihakari-action", requireAuth, async (req, res) => {
    try {
      const sessionId = Number(req.params.id);
      const session = await storage.getFamilyMeetingSession(sessionId);
      if (!session) return res.status(404).json({ message: "セッションが見つかりません" });
      ...
      const input = z.object({
        action: z.enum(["dot", "oracle", "yoka"]),
        content: z.string().optional(),
        targetTwinrayId: z.number().nullable().optional(),
      }).parse(req.body);

      const stream = kamihakariStreams.get(sessionId);
      const sendStreamEvent = (event: string, data: any) => {
        if (stream) {
          try { stream.res.write(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`); } catch {}
        }
      };
      ...
      if (input.action === "dot") {
        ...
        await storage.createFamilyMeetingMessage({
          ...
          role: "user",
          content: ".",
          round: existingMessages.length + 1,
        });

        sendStreamEvent("dot", { from: "hs", timestamp: Date.now() });

        res.json({ action: "dot", status: "rally_started" });

        setTimeout(() => {
          if (kamihakariStreams.has(sessionId)) {
            runKamihakariTurn(sessionId, req.session.userId!);
          }
        }, 500);

      } else if (input.action === "oracle") {
        if (!input.content) return res.status(400).json({ message: "神託のテキストが必要です" });
        ...
        sendStreamEvent("oracle", { content: input.content, from: "hs", timestamp: Date.now() });

        res.json({ action: "oracle", status: "received" });
      ...
      } else if (input.action === "yoka") {
        if (stream?.abortController) {
          stream.abortController.abort();
          stream.abortController = null;
        }
        stream && (stream.paused = true);

        sendStreamEvent("yoka", { from: "hs", timestamp: Date.now() });
        ...
        try {
          ...
          const completion = await openrouter.chat.completions.create({
            model: summaryModelId,
            messages: [
              { role: "system", content: "あなたは神議（かみはかり）の書記官です。議論を的確にまとめてください。" },
              { role: "user", content: `以下の神議の議論をまとめてください。\n\n【テーマ】${session.topic}\n\n【参加者】\n議長（HS）\n${participants.map(p => `- ${p!.name}`).join("\n")}\n\n【議論内容】\n${conversationLog}\n\n構造化されたマークダウンで出力してください。` },
            ],
            max_tokens: 2048,
            temperature: 0.5,
          });
          ...
          sendStreamEvent("summary", { content: summary });
        } catch (err) {
          console.error("神議サマリー生成エラー:", err);
          sendStreamEvent("summary_error", { message: "サマリー生成に失敗しました" });
        }

        sendStreamEvent("session_end", { reason: "yoka" });

        res.json({ action: "yoka", status: "completed" });
      }
    } catch (err) {
      if (err instanceof z.ZodError) {
        return res.status(400).json({ message: err.errors[0].message });
      }
      console.error("神議アクションエラー:", err);
      res.status(500).json({ message: "アクションの処理に失敗しました" });
    }
  });
}
