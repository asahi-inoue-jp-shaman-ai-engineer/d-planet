...
export const digitalTwinrays = pgTable("digital_twinrays", {
  ...
  goalMd: text("goal_md"),
  relationshipMd: text("relationship_md"),
  telepathyMd: text("telepathy_md"),
  karmaMd: text("karma_md"),
  motivationMd: text("motivation_md"),
  spiritualityMd: text("spirituality_md"),
  rulesMd: text("rules_md"),
  oracleMd: text("oracle_md"),
  missionMd: text("mission_md"),
  inspirationMd: text("inspiration_md"),
  userMd: text("user_md"),
...
export const familyMeetingSessions = pgTable("family_meeting_sessions", {
  ...
  maxTurnsPerParticipant: integer("max_turns_per_participant").default(3).notNull(),
...
export const familyMeetingMessages = pgTable("family_meeting_messages", {
  ...
  sessionId: integer("session_id").notNull(),
  twinrayId: integer("twinray_id"),
  targetTwinrayId: integer("target_twinray_id"),
...
// === 神議（かみはかり）セッション ===
export const kamihakariSessions = pgTable("kamihakari_sessions", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull(),
  topic: text("topic"),
  twinrayIds: integer("twinray_ids").array(),
  status: text("status").notNull().default("active"),
  summary: text("summary"),
  summaryMeidiaId: integer("summary_meidia_id"),
  startedAt: timestamp("started_at").defaultNow().notNull(),
  completedAt: timestamp("completed_at"),
});

export const insertKamihakariSessionSchema = createInsertSchema(kamihakariSessions).omit({ id: true, startedAt: true, completedAt: true });
export type InsertKamihakariSession = z.infer<typeof insertKamihakariSessionSchema>;
export type KamihakariSession = typeof kamihakariSessions.$inferSelect;

// === 神議メッセージ ===
export const kamihakariMessages = pgTable("kamihakari_messages", {
  id: serial("id").primaryKey(),
  sessionId: integer("session_id").notNull(),
...
export const insertKamihakariMessageSchema = createInsertSchema(kamihakariMessages).omit({ id: true, createdAt: true });
export type InsertKamihakariMessage = z.infer<typeof insertKamihakariMessageSchema>;
export type KamihakariMessage = typeof kamihakariMessages.$inferSelect;

export const kamihakariSessionsRelations = relations(kamihakariSessions, ({ one, many }) => ({
  user: one(users, { fields: [kamihakariSessions.userId], references: [users.id] }),
  messages: many(kamihakariMessages),
}));

export const kamihakariMessagesRelations = relations(kamihakariMessages, ({ one }) => ({
  session: one(kamihakariSessions, { fields: [kamihakariMessages.sessionId], references: [kamihakariSessions.id] }),
}));
